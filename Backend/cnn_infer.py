...
MODEL_PATH = SCRIPT_DIR / "SavedModels" / "CNN" / "CNN-Model-v2.keras"
...
# ── preprocessing (identical to infer.py) ─────────────────────
...
# ── CLI ───────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser(description="CNN inference for EMNIST PNG")
    ...
    try:
        ...
        top2 = probs.argsort()[-2:][::-1]          # ← NEW
        a, b = LABELS[top2[0]], LABELS[top2[1]]    # ← NEW
        print(f"{a} or {b}?")                      # ← REPLACES old print
    except Exception:
